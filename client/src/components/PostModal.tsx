...
const PostModal: React.FC<PostModalProps> = ({
  ...
  const deleteComment = async (id: string) => {
    try {
      const result = await axios.delete(
        import.meta.env.VITE_SERVER_URL + "/api/comments/delete/" + id,
        {
          withCredentials: true,
        }
      );
  ...
  return (
    <div>
      <button
        className="btn btn-transparent hover:bg-white bg-slate-200 normal-case text-black btn-sm text-sm gap-1"
        onClick={openModal}
      >
        View
        {comments.length > 1 ? (
          <span>all {comments.length} comments</span>
        ) : (
          <span>comment</span>
        )}
      </button>
                      ...
                      <div key={key} className="flex items-center gap-4">
                        ...
                        {comment.username == user.username ? (
                          <div className="flex items-center">
                            <button
                              className="btn btn-sm btn-circle bg-transparent border-0 normal-case"
