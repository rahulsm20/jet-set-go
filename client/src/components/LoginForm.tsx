...
const LoginForm = () => {
  ...
  const onSubmit = async(data:FieldValues) => {
    if(data.username && data.password){
      const result = await axios.post(import.meta.env.VITE_SERVER_URL+"/api/auth/login",{
  ...
  return (
    <div className="flex flex-col justify-start items-center mt-10 gap-5" >
      ...
      >
        ...
        <button className="btn btn-primary normal-case" type="submit"> Login</button>
        <p>
        Don't have an account? <a href="/signup" className="text-blue-500 hover:text-blue-600">Signup here!</a>
        </p>
      </form>
    </div>
  );
